...
#
# Copyright Kirk Steinhorst May 2016
...
SCRAPIv2.0 <- function(smoltData = NULL, Dat = "CollectionDate", Rr = "Rear", Primary = "GenStock", Secondary = NA, passageData = NULL,
                       ...
                       collaps = "Collapse", Run = "output", RTYPE = "W", REARSTRAT = TRUE, alph = 0.1, B = 5000, dateFormat = "%m/%d/%Y")
{
  ...
  # pWild is either proportion wild by stratum or pooled over strata
  thetahat <- function(Fish,passage,pWild){
  ...
  bootsmolt <- function(FishWH,FishDat,LGDdaily){
    ...
    for (b in 2:B) {
      ...
      # Find a bootstrap sample of FishWH BY stratum so we can produce a bootstrap proportion wild
      H <- 0
      for ( h in strats) {
      ...
      tabWH <- table(WHstar$Rear,WHstar$Stratum)
      # Calculate the proportion of smolts that are wild and hatchery
      propwild    <- prop.table(tabWH,margin=2)
      coltotals   <- apply(tabWH,1,sum)
      proportions <- coltotals/sum(coltotals) # This has the pooled H and W proportions
      if(proportions[1] == 1) PrWild <- 1 else {
        if(REARSTRAT == TRUE) PrWild  <- propwild[2,] else PrWild <- proportions[2]
      }

      #if(REARSTRAT == TRUE) PrWild  <- propwild[2,] else PrWild <- proportions[2]
      #if(Proportions[1] == 1) ProWild <- 1 else {
      ...
      #  Now we have bootstrap data for primary fish, passage, and proportion wild--find bootstrap estimates
      eststar <- thetahat(indivStar,dailyStar,PrWild)
      if( !is.na(Secondary) )
  ...
  ests <- thetahat(AllPrime,passdata,ProWild)
  pPropTable <- t(ests[[3]])
