...
#'
#' @param adultData the name of y csv file containing biological data for adults
#' @param windowData the name of the .csv file containing the window count data
...
#' @param Primary the primary category of interest
#' @param Secondary the secondary category of interest
#' @param SizeCut if Secondary = "LGDFLmm", what FL would you like to use to separate Sm versus Lg fish
#' @param alph the alpha used for confidence intervals (e.g., alph = 0.10 -> 90 percent CIs)
#' @param B the number of bootstraps to be performed
#' @param writeThetas would you like to write the thetas?
#' @param writeOutput would you like to write the output files?
#'
...
{
  Fishdata <- read.csv(file = adultData, header = TRUE)
...
if( B > 0) {
  if( is.na(Secondary) ) {
    AllCIs <- AllBoot(Trp,PropPrime,0)
